...
int LinAlg::DiagonalizeSymmetricMatrix(double** u, int dim, int nmax,
                                       double epsilon, double* eigenval,
                                       double** eigenvect) {
  for (int i = 0; i < dim; i++) {
    for (int j = 0; j < dim; j++) {
      eigenvect[i][j] = 0;
...
/*
int LinAlg::DiagonalizeSymmetricMatrix(double** u, int dim, int nmax, double
epsilon, double* eigenval, double** eigenvect)	{
...
int LinAlg::DiagonalizeRateMatrix(double** u, double* pi, int dim,
                                  double* eigenval, double** eigenvect,
                                  double** inveigenvect, int nmax,
                                  double epsilon) {
  double** a = new double*[dim];
  for (int i = 0; i < dim; i++) {
    a[i] = new double[dim];
  ...
  int n =
      DiagonalizeSymmetricMatrix(a, dim, nmax, epsilon, eigenval, eigenvect);
...
double LinAlg::Gauss(std::vector<std::vector<double>> a, int dim,
                     std::vector<std::vector<double>> invu) {
  ...
  std::vector<std::vector<double>> u(dim, vector<double>(dim, 0.0));
  for (int i = 0; i < dim; i++) {
    for (int j = 0; j < dim; j++) {
      u[i][j] = a[i][j];
    }
