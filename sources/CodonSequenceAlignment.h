        ...
        for (int site_codon = 0 ; site_codon < Nsite; site_codon ++ )
        {
            for(int taxa = 0; taxa < Ntaxa; taxa++)
            {
                ...
                if (state != unknown)
                {
                    ...
                    {
                        in[site_codon*3+pos][GetCodonStateSpace()->GetCodonPosition(pos,state)] = 1;
                    }
                }
            }
        }
        ...
        for (int taxa = 0; taxa < Ntaxa; taxa++)
        {
            for ( int site_codon = 0 ; site_codon < Nsite; site_codon ++ )
            {
                ...
                if (state != unknown)
                {
                    for (int pos = 0 ; pos < 3 ; pos ++ )
                    {
                        stat_container[GetCodonStateSpace()->GetCodonPosition(pos,state)]++;
                        total++;
                    }
                }
            }
    ...
    void dinuc_usage(double** stat_container)
    {
        ...
        for(int taxa = 0; taxa < Ntaxa; taxa++)
        {
            for (int site_codon = 0; site_codon < Nsite; site_codon++)
            {
                ...
                {
                    next_state = Data[taxa][site_codon+1]; //GetState(taxa,site_codon+1);
                }
                if (state != unknown && next_state != unknown)
                {
                    stat_container[GetCodonStateSpace()->GetCodonPosition(0, state)][GetCodonStateSpace()->GetCodonPosition(1, state)]++;
                    stat_container[GetCodonStateSpace()->GetCodonPosition(1, state)][GetCodonStateSpace()->GetCodonPosition(2, state)]++;
                    stat_container[GetCodonStateSpace()->GetCodonPosition(2, state)][GetCodonStateSpace()->GetCodonPosition(0, next_state)]++;
                    tot += 3;
                }
    ...
    void dinuc31_usage(double** stat_container)
    {
        ...
        for (int taxa = 0; taxa < Ntaxa; taxa++)
        {
            for (int site_codon = 0; site_codon < Nsite; site_codon++)
            {
                ...
                if(site_codon < Nsite-1)
                {
                    int next_state = Data[taxa][site_codon+1]; //GetState(taxa,site_codon+1);
                    if(state != unknown && next_state != unknown)
                    {
                        stat_container[GetCodonStateSpace()->GetCodonPosition(2, state)][GetCodonStateSpace()->GetCodonPosition(0, next_state)]++;
                        tot++ ;
...
// CpG+meandiff
    ...
    void  CpG3_mean(double* stat_container)
    {
        ...
        for (int site_codon = 0 ; site_codon < Nsite-1; site_codon ++ )
        {
            ...
            {
                int state = Data[taxa][site_codon]; //GetState(taxa, site_codon);
                int next_state = Data[taxa][site_codon+1]; //GetState(taxa, site_codon);
                if (state != unknown)
                {
        ...
        for (int site_codon = 0 ; site_codon < Nsite; site_codon ++ )
        {
            for (int aa = 0 ; aa < Naa; aa++)
            {
                ...
                for(int codon = 0; codon < GetCodonStateSpace()->GetNstate(); codon++)
                {
                    ...
                    if(aa == GetCodonStateSpace()->Translation(codon) && in[site_codon][codon] == 1)
                    {
                        NDiffsynPerAA++;
...
//    varRatio = varRatio - (meanRatio * meanRatio);
    ...
    {
        int tot = GetCodonStateSpace()->GetNstate()*GetCodonStateSpace()->GetNstate() ;
        for (int codon1 = 0 ; codon1 < GetCodonStateSpace()->GetNstate() ; codon1 ++ )
        {
            for (int codon2 = 0 ; codon2 < GetCodonStateSpace()->GetNstate(); codon2 ++ )
            {
    ...
    //    int AGC = GetCodonStateSpace()->GetState("AGC"); // S
        ...
        mean /= static_cast<double> (Nsite_wonR);
        var /= static_cast<double> (Nsite_wonR);
        var = var - (mean * mean);
                ...
                for (int taxa2 = taxa1 + 1; taxa2 < Ntaxa; taxa2++)
                {
                    ...
                    if(state_seq1 != unknown && state_seq2 != unknown)
                    {
                        for(int pos = 0 ; pos < 2 ; pos++ )
                        {
                            ...
                            int posa1 = GetCodonStateSpace()->GetCodonPosition(pos,state_seq1);
                            int posa2 = GetCodonStateSpace()->GetCodonPosition(pos+1,state_seq1);
                            int posb1 = GetCodonStateSpace()->GetCodonPosition(pos,state_seq2);
                            int posb2 = GetCodonStateSpace()->GetCodonPosition(pos+1,state_seq2);

                            if ((posa1 == 1 && posb1 == 2 && posa2 == 3 && posb2 == 2) || (posa1 == 3 && posb1 == 2 && posa2 == 1 && posb2 == 2))  //CpG<->TpG
                            {
                                invec[0]++;
                                invec[3]++;
                            }
                            else if ((posa1 == 1 && posb1 == 2 && posa2 == 1 && posb2 == 0) || (posa1 == 1 && posb1 == 0 && posa2 == 1 && posb2 == 2))    //CpG<->CpA
                            {
                                invec[1]++;
                                invec[3]++;
                            }
                            else if ((posa1 == 0 && posb1 == 2 && posa2 == 3 && posb2 == 2) || (posa1 == 3 && posb1 == 2 && posa2 == 0 && posb2 == 2))    //ApG<->TpG
                            {
                                invec[2]++;
                                invec[3]++;
                            }
                    ...
                    if(state_seq1 != unknown && state_seq2 != unknown && next_state_seq1 != unknown &&  next_state_seq1 != unknown)
                    {
                        int posa1 = GetCodonStateSpace()->GetCodonPosition(2,state_seq1);
                        int posa2 = GetCodonStateSpace()->GetCodonPosition(0,next_state_seq1);
                        int posb1 = GetCodonStateSpace()->GetCodonPosition(2,state_seq2);
                        int posb2 = GetCodonStateSpace()->GetCodonPosition(0,next_state_seq2);
...
//pairwise stat_containers
                ...
                for (int taxa2 = taxa1 + 1; taxa2 < Ntaxa; taxa2++)
                {
                    ...
                    if(state_seq1 != unknown && state_seq2 != unknown)
                    {
                        for(int pos = 0 ; pos < 3 ; pos++ )
                        {
                            ...
                            if ((posa == 0 && posb == 1 ) || (posa == 1 && posb == 0))  // ac_ca
                            {
                                invec[0]++;
                                invec[6]++;
                            }
                            ...
                            else if ((posa == 3 && posb == 2 ) || (posa == 2 && posb == 3))    //tg_gt
                            {
                                invec[5]++;
                                invec[6]++;
        ...
        for (int site_codon = 0; site_codon < Nsite; site_codon++)
        {
            for (int taxa1 = 0; taxa1 < Ntaxa-1; taxa1++)
            {
                for (int taxa2 = taxa1 + 1; taxa2 < Ntaxa; taxa2++)
                {
                    ...
                    if(state_seq1 != unknown && state_seq2 != unknown)
                    {
                        int posa = GetCodonStateSpace()->GetCodonPosition(0,state_seq1);
                        int posb = GetCodonStateSpace()->GetCodonPosition(0,state_seq2);
                        if ((posa == 0 && posb == 1 ) || (posa == 1 && posb == 0))  // ac_ca
                        {
                            invec[0]++;
                            invec[6]++;
        ...
        for (int site_codon = 0; site_codon < Nsite; site_codon++)
        {
            for (int taxa1 = 0; taxa1 < Ntaxa-1; taxa1++)
            {
                for (int taxa2 = taxa1 + 1; taxa2 < Ntaxa; taxa2++)
                {
                    ...
                    if(state_seq1 != unknown && state_seq2 != unknown)
                    {
                        int posa = GetCodonStateSpace()->GetCodonPosition(1,state_seq1);
                        int posb = GetCodonStateSpace()->GetCodonPosition(1,state_seq2);
                        if ((posa == 0 && posb == 1 ) || (posa == 1 && posb == 0))  // ac_ca
                        {
                            invec[0]++;
                            invec[6]++;
    ...
    void nuc3_pairwise30(int* invec)
    {
        for (int i = 0 ; i < 7; i ++ )
        {
            invec[i] = 0 ;
    ...
    void nuc3_pairwise50(int* invec)
    {
        for (int i = 0 ; i < 7; i ++ )
        {
            invec[i] = 0 ;
    ...
    double dinuc_site_comphet()
    {
        ...
        for (int site_codon=0; site_codon<Nsite; site_codon++)
        {
            for (int taxa=0; taxa<Ntaxa; taxa++)
            {
                ...
                if (state != unknown)
                {
                    sitefreq[site_codon][GetCodonStateSpace()->GetCodonPosition(0,state)]++;
                    sitefreq[site_codon][GetCodonStateSpace()->GetCodonPosition(1,state)]++;
                    sitefreq[site_codon][GetCodonStateSpace()->GetCodonPosition(2,state)]++;
                }
            }
        }
