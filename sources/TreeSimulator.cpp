...
void TreeSimulator::RegisterSubTreeSim(int NodeIndex, int site_nuc, int nucTo) {
      ...
      treeEvoStats->dinuc_stat[3][treeEvoStats->GetDinucContext(
          nucposFrom[site_nuc_To - 1], nucposFrom[site_nuc_To])]
                              [treeEvoStats->GetDinucContext(
                                  nucposTo[site_nuc_To - 1],
                                  nucposTo[site_nuc_To])]++;  //
      treeEvoStats->dinuc_stat[0][treeEvoStats->GetDinucContext(
          nucposFrom[site_nuc_To - 1], nucposFrom[site_nuc_To])]
                              [treeEvoStats->GetDinucContext(
                                  nucposTo[site_nuc_To - 1],
                                  nucposTo[site_nuc_To])]++;  //
      ...
      treeEvoStats->dinuc_stat[3][treeEvoStats->GetDinucContext(
          nucposFrom[site_nuc_To - 1], nucposFrom[site_nuc_To])]
                              [treeEvoStats->GetDinucContext(
                                  nucposTo[site_nuc_To - 1],
                                  nucposTo[site_nuc_To])]++;  //
      treeEvoStats->dinuc_stat[1][treeEvoStats->GetDinucContext(
          nucposFrom[site_nuc_To - 1], nucposFrom[site_nuc_To])]
                              [treeEvoStats->GetDinucContext(
                                  nucposTo[site_nuc_To - 1],
                                  nucposTo[site_nuc_To])]++;  //
  ...
  // lparam->codonstatespace->GetCodonFromDNA(nucposTo[3], nucposTo[4],
  // nucposTo[5]);
...
void TreeSimulator::ComputeRecursiveSimulation(Link* from) {
  ...
  if (from->isRoot()) {
    ...
    while (time < blength) {
      ...
      //
      //                testcummul =
      //                submatrix->GetMutRate(FromNodeIndex,site_nuc,nucTo);
      //
      //            }
      int k = 0;
      while (testcummul < u) {
...
void TreeSimulator::GetSampleAncestralCodonSequence(int FromNodeIndex,
                                                    int interval) {
  for (int site_codon = 0; site_codon < lparam->Nsite_codon; site_codon++) {
