...
LocalParameters::LocalParameters(GlobalParameters *gparam) {
  ...
  this->mapUsedAncSummaries.insert(gparam->mapUsedAncSummaries.begin(),
                                   gparam->mapUsedAncSummaries.end());
...
void LocalParameters::GetGTR1() {
  ...
  for (int i = 0; i < 4 - 1; i++) {
    for (int j = i + 1; j < 4; j++) {
      this->gtnr1[i][j] =
          (this->nucp1[j] * this->nucrrnr1[i][j]) / MutationNormFactor1;
    }
  }
}
...
void LocalParameters::GetGTR2() {
  ...
  for (int i = 0; i < 4 - 1; i++) {
    for (int j = i + 1; j < 4; j++) {
      this->gtnr2[i][j] =
          (this->nucp2[j] * this->nucrrnr2[i][j]) / MutationNormFactor1;
    }
  }
}
...
void LocalParameters::readLocalInstructions() {
  ...
  while (iss >> s) {
    if (s == "-d") {
      ...
      //                    for (unsigned int param_i = 0 ; param_i <
      //                    gparam->NParam; param_i++){
...
void LocalParameters::SetRootBetweenInAndOutGroup() {
  if (refTree->GetLCA(taxa_a, taxa_b)->isRoot()) {
    ...
    } else if (refTree->GetLCA(taxa_a, taxa_b)
                       ->Next()
                       ->Next()
                       ->Out()
                       ->GetNode()
                       ->GetName() != taxa_a &&
               refTree->GetLCA(taxa_a, taxa_b)
                       ->Next()
                       ->Next()
                       ->Out()
                       ->GetNode()
...
void LocalParameters::SetCurrentParametersFromPosterior(
  ...
  for (int param_i = 0; param_i < this->NusedParam; param_i++) {
    if (arrParam[param_i] == "chainID") {
      this->MCMCpointID = static_cast<int>(posterior[pointID][param_i]);
      std::cerr << "chainID " << this->MCMCpointID << "\n";
    } else if (arrParam[param_i] == "root") {
      this->percentFromOutGroup = posterior[pointID][param_i];
      ...
      this->fitGC = posterior[pointID][param_i];
      if (this->fitGC >= 1.0) {
      ...
      this->fitCpG = posterior[pointID][param_i];
      if (fitCpG >= 1.0) {
      ...
      this->fitTpA = posterior[pointID][param_i];
      if (fitTpA >= 1.0) {
...
int LocalParameters::readBayescodeParametersMutSelAAC(int it) {
  this->MCMCpointID = it;
  // set parameters : posterior specific
  ifstream is((this->chain + ".pvalues").c_str());
  if (!is) {
  ...
  assert(this->Nsite_codon == atoi(tmp.c_str()));
