...
//                    }
                    ...
                    iss >> s ; this->taxa_gtr1_a = s;
                    iss >> s ; this->taxa_gtr1_b = s;
                    iss >> s ; this->taxa_gtr1_c = s;

                    iss >> s; this->nucp1[0] = atof(s.c_str());
                    iss >> s; this->nucp1[1] = atof(s.c_str());
                    iss >> s; this->nucp1[2] = atof(s.c_str());
                    iss >> s; this->nucp1[3] = atof(s.c_str());

                    iss >> s; this->nucrr1[0] = atof(s.c_str());
                    iss >> s; this->nucrr1[1] = atof(s.c_str());
                    iss >> s; this->nucrr1[2] = atof(s.c_str());
                    iss >> s; this->nucrr1[3] = atof(s.c_str());
                    iss >> s; this->nucrr1[4] = atof(s.c_str());
                    iss >> s; this->nucrr1[5] = atof(s.c_str());

                    cerr << " fixgtr1 " << this->fixgtr1 << "\n";
                    cerr << this->taxa_gtr1_a << "\t" << this->taxa_gtr1_b <<"\t" << this->taxa_gtr1_c << "\n";
                    for (int v = 0 ; v < Nnuc ; v ++ ) { cerr << this->nucp1[v] << "\t"; }
                    cerr << "\n";
                    for (int v = 0 ; v < Nnucrr ; v ++ ) { cerr << this->nucrr1[v] << "\t"; }
                    cerr << "\n";

                } else if (s =="-fixgtr2"){
                    this->fixgtr2 = 1;

                    iss >> s ; this->taxa_gtr2_a = s;
                    iss >> s ; this->taxa_gtr2_b = s;
                    iss >> s ; this->taxa_gtr2_c = s;

                    iss >> s; this->nucp2[0] = atof(s.c_str());
                    iss >> s; this->nucp2[1] = atof(s.c_str());
                    iss >> s; this->nucp2[2] = atof(s.c_str());
                    iss >> s; this->nucp2[3] = atof(s.c_str());

                    iss >> s; this->nucrr2[0] = atof(s.c_str());
                    iss >> s; this->nucrr2[1] = atof(s.c_str());
                    iss >> s; this->nucrr2[2] = atof(s.c_str());
                    iss >> s; this->nucrr2[3] = atof(s.c_str());
                    iss >> s; this->nucrr2[4] = atof(s.c_str());
                    iss >> s; this->nucrr2[5] = atof(s.c_str());


                    cerr << " fixgtr2 " << this->fixgtr2 << "\n";
                    cerr << this->taxa_gtr2_a << "\t" << this->taxa_gtr2_b << "\t" << this->taxa_gtr2_c << "\n";
                    for (int v = 0 ; v < Nnuc ; v ++ ) { cerr << this->nucp2[v] << "\t"; }
                    cerr << "\n";
                    for (int v = 0 ; v < Nnucrr ; v ++ ) { cerr << this->nucrr2[v] << "\t"; }
                    cerr << "\n";
