...
CodonSequenceAlignment::CodonSequenceAlignment(SequenceAlignment* from,
                                               bool force_stops,
...
/*void CodonSequenceAlignment::ToStream(ostream& os)	{
        ...
        for (int i=0; i<Ntaxa; i++)	{
                os << taxset->GetTaxon(i);
                for (unsigned int j=0; j< 5 + max -
taxset->GetTaxon(i).length(); j++)	{ os << ' ';
                }
                for (int j=0; j<Nsite; j++)	{
                        int tempCodonState = Data[i][j];
                        int tempAAState =
GetCodonStateSpace()->Translation(tempCodonState); if (tempCodonState ==
unknown)	{ os << '-';
                        }
                        else {
