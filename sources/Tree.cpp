...
void Tree::ToStreamWithIndices(ostream& os, const Link* from) {
        ...
        if (!from->isLeaf())	{
                os << '(';
                for (const Link* link=from->Next(); link!=from;
link=link->Next())	{ ToStreamwithIndices(os,link->Out()); if (link->Next()
!= from)	{ os << ',';
                        }
                }
                os << ')';
        ...
        if (!from->isRoot())	{
                os << from->GetIndex() << ':' << from->GetBranch()->GetIndex()
<< from->Out()->GetIndex();
        }
}
...
void Tree::Print(ostream& os, const Link* from)	const {
        ...
        if (!from->isLeaf())	{
                os << '(';
                for (const Link* link=from->Next(); link!=from;
link=link->Next())	{ Print(os,link->Out()); if (link->Next() != from)
{ os << ',';
                        }
                }
                os << ')';
...
int Tree::DrawSubTree(Link*& down, Link*& up)	{
        ...
        int n = 0;
        for (const Link* link= root->Next(); link!=root; link=link->Next())
{ n += CountNodes(link->Out()); n-=4;
        }
        int choose = (int) (n * rnd::GetRandom().Uniform());
        down = 0;
        up = 0;
        for (const Link* link= root->Next(); link!=root; link=link->Next())
{ if (! down)	{ choose++; down = ChooseNode(link->Out(),up,choose);
                }
        }
        if ((! down) || (down->isRoot()))	{
                std::cerr << "error in tree::drawsubtree: down\n";
