...
double Random::Uniform() {
  ...
  // caller but this doesn't increase the randomness, and slows down the
  // generator by as much as 25%.  So I omit these operations here.
...
void Random::DrawFromUrn (int* tab , int n, int N)      {       // draw n out of
N
...
double Random::logGamma(double alpha) {
  ...
  return log(tot * sqrt(2 * Pi)) - alpha - 6.5 +
         (alpha - 0.5) * log(alpha + 6.5);
}
