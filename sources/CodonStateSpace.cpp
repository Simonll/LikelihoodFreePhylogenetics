...
CodonStateSpace::CodonStateSpace(GeneticCodeType type) {
  ...
  if (code == Universal) {
    ...
    CodeAADeg = new int[Naa];
    for (int i = 0; i < Naa; i++) {
      CodeAADeg[i] = UniCodeAADeg[i];
    }
...
std::string CodonStateSpace::GetState(int codon) {
  ...
  if (codon == -1) {
    ...
    s << DNAletters[GetCodonPosition(0, codon)]
      << DNAletters[GetCodonPosition(1, codon)]
...
bool CodonStateSpace::isStop(int pos1, int pos2, int pos3) {
  ...
  int l = 0;
  while ((l < Nstop) && ((pos1 != StopPos1[l]) || (pos2 != StopPos2[l]) ||
                         (pos3 != StopPos3[l]))) {
    l++;
  }
  return (l < Nstop);
...
int CodonStateSpace::GetCodonFromDNA(int pos1, int pos2, int pos3) {
  ...
  int l = 0;
  while ((l < GetNstate()) && ((pos1 != GetCodonPosition(0, l)) ||
                               (pos2 != GetCodonPosition(1, l)) ||
...
int CodonStateSpace::GetDegFromAA(int aa) { return CodeAADeg[aa]; }

int CodonStateSpace::GetDegFromCodon(int codon) {
  if (codon != -1) {
    return CodeAADeg[Translation(codon)];
  } else {
    return -1;
  }
}
...
{ if ((pos1 == unknown) || (pos2 == unknown) || (pos3 == unknown))	{ return
unknown;
        }
        int l = 0;
        while ((l<Ncodon) && ((pos1 != codonpos[0][l]) || (pos2 !=
