...
Regression::Regression(GlobalParameters* gparam, Posterior* post) {
  ...
  std::vector<std::vector<double>> B_HAT(
      gparam->NusedSummaries + 1, vector<double>(gparam->NusedParam, 0.0));
}
...
void Regression::ComputeMultipleRegression() {
  ...
  for (int summaries_i = 0; summaries_i < gparam->NusedSummaries + 1;
       summaries_i++) {
  ...
  for (int summaries_i = 0; summaries_i < gparam->NusedSummaries + 1;
    ...
    for (int summaries_j = 0; summaries_j < gparam->NusedSummaries + 1;
         summaries_j++) {
      double sum = 0.0;
      for (int simu_i = 0; simu_i < gparam->threshold; simu_i++) {
  ...
  for (int summaries_i = 0; summaries_i < gparam->NusedSummaries + 1;
    ...
    for (int summaries_j = 0; summaries_j < gparam->NusedSummaries + 1;
         summaries_j++) {
      for (int param_i = 0; param_i < gparam->NusedParam; param_i++) {
        B_HAT[summaries_i][param_i] +=
            X_TWXinv[summaries_i][summaries_j] * X_TWY[summaries_j][param_i];
      }
    }
  }
}
